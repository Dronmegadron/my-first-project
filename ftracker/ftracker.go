...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ДОПУСТИМ ГОТОВО
	if duration == 0 || height == 0 || action == 0 {
		return 0
	}
	...
	// Расход калорий по формуле
	return ((walkingCaloriesWeightMultiplier * weight) +
		(math.Pow(speed, 2) / heightInMeters * walkingSpeedHeightMultiplier * weight)) *
		duration * minInH
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ВРОДЕ ТОЖЕ ГОТОВО
	if duration == 0 || lengthPool == 0 || countPool == 0 || weight == 0 {
		return 0
	}
